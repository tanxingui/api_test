...
class MyAssert(BaseApi):
    ...
    def assert_expect(self, assert_list):
        ...
        if assert_list == []:
            ...
            # 对字符串进行类型装换的时候，最好使用ast.literal_eval()函数
    ...
    def json_source(self, assert_dict):
        if assert_dict["actual"]["data_sources"] == "last_request":
            ...
            response_json = self.assert_request(assert_dict["actual"]["data_sources"],
                                                assert_dict["actual"]["body_key"])
    ...
    def assert_all(self, response_json, assert_dict):
        ...
        if actual == False:
            return False
        elif assert_dict["type"] == "" or assert_dict["expected"] == "":
            return False
